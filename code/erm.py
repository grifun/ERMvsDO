...
def eps_nash_erm(game:Game, C:float = 1e-11, init_algorithm:str = "bounds", maxiter:int = 20, eps:float=1e-6):
    ...
    for t in range(maxiter):
        ...
        # Step (e)
        game_value1,_,_ = valueOracle(new_a_s, b_s, game)
        game_value2,_,_ = valueOracle(a_s, b_s, game)
        game_value3,a_mixed_strategy,b_mixed_strategy = valueOracle(new_a_s, new_b_s, game)
...
def eps_nash_half_infinite(action_list:ActionSet, action_space:HyperBlock, game:Game, C:float, eps:float, player:str="a"):
    ...
    first_response, val = get_response(action_list, mixed_strategies[-1], game)
    ...
    for t in range(1, T):
        ...
        curr_response, curr_val = get_response(action_list, curr_mixed_strategy, game)
