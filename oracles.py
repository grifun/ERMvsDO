...
def compute_matrix(a_s:np.array, b_s:np.array, u:callable):
...
def optimal_mixed_strategy(matrix:np.array, player='a', lp_solver="simplex"):
...
def already_exists(xs:np.array, x:np.array, eps:float):
...
def reduce_strategies(xs:np.array, p:np.array, ys:np.array, q:np.array, epsilon=1e-8):
