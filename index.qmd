...
- the final action $a$ ($b$) is i.i.d drawn from $X$ ($Y$)
...
"Let $G = (A = {a_1, . . . , a_n}, Y, u)$ be a zero-sum game where $|A| = n$ and $Y$ is possibly infinite, and let $\epsilon > 0$. Then, the subroutine, executed with the parameter $\epsilon$, finds an $O(\epsilon)$-Nash equilibrium, after $V = O(\log \; n/\epsilon^2)$ iterations." - Tested on Rock-Paper-Scissors (finite)
...
- a possible tradeoff (compared to the DO)
